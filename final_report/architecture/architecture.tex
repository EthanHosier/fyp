...
This chapter describes the engineering side of the tool. The methodology chapter argued for why each algorithm is correct; this chapter argues for why each one is fast. It walks through the pipeline in order: event capture in the plugin (\S\ref{sec:arch-events}), shadow-repo reconstruction and worktree allocation (\S\ref{sec:arch-shadow}), per-checkpoint metrics calculation (\S\ref{sec:arch-metrics}), the Eclipse JDT refactoring engine that every synthesiser leans on (\S\ref{sec:arch-jdt}), and the four counterfactual-trajectory synthesis strategies (\S\ref{sec:arch-synthesis}). The split of the pipeline into an expensive offline phase and a cheap replay phase is described in \S\ref{sec:arch-phases}, and the dashboard that consumes the resulting report in \S\ref{sec:arch-dashboard}.
...
\begin{itemize}
  \item \texttt{ide-plugin/} -- an IntelliJ plugin that records the developer's session as a stream of typed events and uploads it to the analysis server.
...
The Intellij plugin is responsible for capture the Intellij events which are emitted during the user's refactoring process. The captured event types cover editor activity (files opened, closed, saved, modified, renamed, moved), refactoring activity (refactoring start / finish), build activity (build start / finish), test activity (test-run start / finish with pass and fail counts), git activity (one event per new commit observed in the underlying repository), and the session lifecycle.

Editor activity is the noisiest source, so keystroke-level events are not emitted directly. A debouncing layer accumulates keystrokes into an \emph{edit burst}: when a quiet window elapses without further typing, a single burst event is emitted carrying the file's post-edit contents and the structured set of program elements touched.
