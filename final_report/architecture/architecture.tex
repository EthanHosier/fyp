...
This split matters because weights only enter at Phase B: changing them won't break any cached results from Phase A. The sensitivity experiment of Chapter~\ref{ch:results} exploits this directly: $12 \times 9 \times 45 = 4{,}860$ scoring configurations are evaluated against the same cached \texttt{PhaseAResult} corpus in roughly $2.6$ seconds of wall-clock time, because each configuration only re-runs \texttt{ReportAssembler}. Two Gradle entrypoints make the boundary visible at the CLI layer: \texttt{:analysis:phaseA} dumps a \texttt{PhaseAResult} JSON to disk, and \texttt{:analysis:phaseB} re-runs the cheap second phase against a Phase-A dump and a chosen \texttt{ScoringConfig}.
