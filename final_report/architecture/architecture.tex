...
Refactorings need to identify their target, but line numbers shift when code changes. The pipeline addresses this by using line and column coordinates plus a canonical hash of the surrounding AST subtree. This hash is robust to formatting and line shifts: even if the method moves from line 42 to line 47, the hash can still locate it. The same hash is computed on both the host and JDT sides, ensuring they identify the same AST node.
