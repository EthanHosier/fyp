...
The experiment is run against three session types: the $45$ hand-labelled injection sessions of \S\ref{sec:results-divergence}, the $30$ user-study sessions of \S\ref{sec:results-userstudy}, and the $48$ agent extension sessions of \S\ref{sec:results-userstudy-agent}. These differ in their per-session divergence-point profiles. The injection sessions have at most $5$ detected divergence points per session and concentrate on single-kind exercises. The user-study sessions have up to $15$ detected divergence points per session across more natural sessions. The agent sessions have at most $4$ detected divergence points per session across the eight agent configurations, partly because the detector is designed around human IDE edit patterns rather than agent file-edit patterns (\S\ref{sec:results-userstudy-agent}).

Rank-based statistics ($\tau$-b, top-$N$) are computed on the \emph{rankable subset} of each session set: sessions whose analysis report contains $\geq 2$ detected divergence points. Both metrics are mechanically $1.0$ on smaller rankings (a single-item ranking cannot be perturbed and a zero-item ranking is undefined), so including those sessions inflates the headline figures without testing anything. This leaves $15$ of $45$ injection sessions, $25$ of $30$ user-study sessions, and $20$ of $48$ agent sessions in scope for the rank statistics. The magnitude-recovery statistics in \S\ref{sec:results-ablation} (sum-over-sum, mean recovery) continue to use the full session sets, since magnitudes are well-defined on every session including those with $\leq 1$ DP.
...
Table~\ref{tab:results-sensitivity-headline} reports top-$1$ stability across the rankable subsets of the three session types. The user-study set is the strongest test of ranking stability because it carries the largest rankable subset ($25$ of $30$ sessions), the longest per-session rankings, and a low ceiling-hit rate. The injection rankable subset is the smallest ($15$ sessions); its top-$1$ stability is high but rests on a smaller per-session corpus. The agent rankable subset ($20$ sessions) sits between the two, although $21.6\%$ of its divergence points hit the score-clamp ceiling before perturbation, which limits how much weight perturbations can change the observed rankings. On the user-study rankable subset, the top-$1$ recommendation is preserved in $2{,}823$ of $2{,}925$ perturbation cases ($96.5\%$).
...
\hline
\textbf{session set (rankable)} & \textbf{cases} & \textbf{$\tau = 1.0$} & \textbf{top-$1$ $= 1$} & \textbf{$\tau < 0$} & \textbf{ceiling-hit rate} \\
\hline
Injection ($15$)         & $1{,}755$ & $30.7\%$ & $99.0\%$ & $1.3\%$ & $8.9\%$ \\
\textbf{User study ($25$)} & $2{,}925$ & $41.9\%$ & $\mathbf{96.5\%}$ & $1.0\%$ & $\mathbf{6.4\%}$ \\
Agent ($20$)              & $2{,}340$ & $72.4\%$ & $96.3\%$ & $3.0\%$ & $21.6\%$ \\
\hline
\end{tabular}
\caption{Single-knob sensitivity across the rankable subsets of the three session types ($\geq 2$ detected divergence points per session). The user-study set is the strongest test of ranking stability because it carries the largest rankable subset ($25$ of $30$ sessions) and the longest per-session rankings. The $\tau = 1.0$ shares are lower than the headline figures from earlier corpus-wide reports because the rankable filter removes the mechanically-saturated $\leq 1$-DP sessions; what remains is genuine perturbation response. Even so, the top-ranked divergence point is preserved in $96.5\%$ of user-study perturbation cases.}
\label{tab:results-sensitivity-headline}
\end{table}

\paragraph{Sources of ranking instability.}
Table~\ref{tab:results-sensitivity-knobs} reports how often each individual weight changes the top-ranked divergence point on the rankable user-study subset. Six of the seven process-side weights change the top-$1$ result in between $5.3\%$ and $10.7\%$ of cases. The lag weight $W_{\textsc{lag}}$ is the exception: it changes the top-$1$ result in only $1$ of $225$ cases ($0.4\%$), in line with the cross-set leave-one-out finding in \S\ref{sec:results-ablation} that the lag term contributes magnitude on the injection set but does not noticeably reshape the user-study ranking. Each cleanliness sub-weight changes at most $0.9\%$ of cases, and three of the six do not change the top-ranked item at all. The cleanliness-gain weight $W_{\textsc{g}}$ has the largest effect ($24$ of $225$ cases, $10.7\%$), followed by the manual-when-IDE weight $W_{\textsc{mi}}$ ($17$ cases, $7.6\%$), the skipped-tests weight $W_{\textsc{st}}$ ($15$, $6.7\%$), and the commit-gap weight $W_{\textsc{cg}}$ ($15$, $6.7\%$). The length-bonus and broken-build weights are close behind ($14$ and $12$). In practice, this means the user-study ranking is shaped mostly by six of the seven process-side terms. The cleanliness composite often changes both paths by a similar amount, so changing its internal weights rarely changes which divergence point is ranked first.
...
\hline
\end{tabular}
\caption{Per-knob top-$1$ disruption counts on the user-study rankable subset (25 sessions $\times$ 9 perturbation factors $=$ 225 cases per weight). Six of the seven process-side weights account for almost every top-$1$ shift; the lag weight produces only $1$ disruption on the user-study rankable subset, and the cleanliness sub-weights produce at most $0.9\%$ disruption each. The cleanliness-gain weight $W_{\textsc{g}}$ has the largest single-weight effect on this corpus.}
...
The multi-knob result is meaningfully weaker than the single-knob headline. On the user-study rankable subset, the top-$1$ recommendation changes on $15.4\%$ of joint-perturbation samples, against $3.5\%$ under single-knob, and mean $\tau$ falls to $0.586$. The injection rankable subset stays high on top-$1$ ($95.7\%$) but its multi-knob mean $\tau$ falls to $0.385$, lower than the user-study figure because the injection rankings are short and joint perturbation can reshuffle most of the tail with a single unlucky draw. The agent rankable subset sits between the two on mean $\tau$ ($0.645$), with top-$1$ stability at $82.5\%$. The formula is robust to small joint perturbations and degrades smoothly with larger ones. It is not robust to arbitrary combinations of weights, even within the realistic range we tested.
...
\hline
\end{tabular}
\caption{Multi-knob Monte Carlo robustness across the rankable subsets ($200$ samples per session, log-normal $\sigma = \ln 2$). The user-study set carries the headline because it has the largest rankable subset and the longest per-session rankings. Multi-knob top-$1$ stability is $\sim 12$ percentage points lower than the single-knob result on the same set. Top-$3$ and top-$5$ track top-$1$ on the agent set ($78.3\%$ and $75.0\%$) but drop on the user-study set ($61.7\%$ and $59.5\%$) where per-session rankings are longer and the tail is more reshuffleable. The injection top-$3$/top-$5$ figures fall sharply because injection rankings have at most $5$ items, so a single tail flip propagates through top-$3$ and top-$5$ simultaneously.}
\label{tab:results-sensitivity-mc}
\end{table}

\paragraph{Ceiling hits and score-clamp bias.}
One concern is whether the $\tau$ and top-$N$ numbers reflect the $[0, 100]$ clamp on the process score rather than true stability. When both user and alt scores hit the ceiling, their magnitude is zero regardless of weights, which would inflate $\tau$. The ceiling-hit rate is therefore tracked per session. The right-hand column of Table~\ref{tab:results-sensitivity-headline} shows the three rankable subsets differ substantially. The injection rankable subset has an $8.9\%$ ceiling-hit rate. The agent rankable subset has $21.6\%$, because many agent sessions push the score close to the ceiling on the easier playbook sessions (the S4 no-op session in particular runs at $J = 45$ across every agent cell). The user-study rankable subset sits lowest at $6.4\%$. The user-study set is therefore the cleanest benchmark for the perturbation statistics: its $\tau$ and top-$N$ values mostly reflect genuine responses to the weight changes, not the score ceiling.

\paragraph{Caveats and scope.}
There are three main limitations. First, the experiment establishes that the formula is stable near the production weights. It does not establish that the production weights themselves are uniquely correct, only that small changes to them mostly preserve the ranking. To properly validate this, we'd need to refit the weights against an external outcome dataset, e.g.\ longitudinal code-quality measurements, but no such dataset exists for this problem. Second, the multi-knob test is centred around the production weights and covers the neighbourhood within roughly $\pm 2\sigma$ (about $\times 0.25$ to $\times 4$). The formula visibly degrades on the user-study rankable subset even inside this range ($\tau = 0.586$ mean), so the robustness claim is local rather than global. Third, the rank statistics are reported on the rankable subset of each set ($N = 15 / 25 / 20$); the smaller injection rankable subset is a known statistical-power limit on the per-set rank comparison, though the magnitude-recovery statistics in \S\ref{sec:results-ablation} continue to use all $45$ injection sessions. We document these limits as threats to validity in \S\ref{sec:results-scoped-out} and the discussion chapter. The chapter does not claim more than the experiments support.

\subsection{Ablation sweep}
\label{sec:results-ablation}

The sensitivity sweep shows that the ranking is fairly stable under weight changes on the rankable subset. The ablation sweep asks a different question: whether each process-side term is actually doing useful work. Following the design in \S\ref{sec:methodology-ablation-design}, it tests all $2^7 = 128$ subsets of the seven process-side weights. Across the $45$ injection sessions, this gives $5{,}760$ ablation cases. The cleanliness sub-weights are left at their production values because the sensitivity sweep showed that changing them almost never reshuffles the ranking - including them would increase the number of variants from $128$ to $8{,}192$ for little information gain. The ablation tables in this subsection report magnitude metrics (sum-over-sum and mean recovery) on all $45$ injection sessions, since magnitudes are well-defined on every session regardless of per-session DP count; the cross-set rank comparison in Table~\ref{tab:results-ablation-user-loo} reports $\tau$ on the rankable subsets of injection ($15$ sessions) and user-study ($25$ sessions). This gives a useful sanity check: when every process-side term is removed, sum-over-sum recovery falls to exactly $0.000$; under the full formula it rises to $1.000$.

\paragraph{Monotone by active-term count.}
Table~\ref{tab:results-ablation-monotone} groups the $5{,}760$ ablation cases by how many process-score terms are active. Sum-over-sum recovery rises steadily as more terms are restored, from $0.000$ with every process term removed to $1.000$ under the full formula. The important sanity check is the all-stripped case: when every process term is zeroed, total magnitude across all $45$ sessions drops to zero. This means there is no baseline magnitude independent of the weights; the reported divergence magnitudes come from the process terms themselves.
...
\begin{tabular}{rrr}
\hline
\textbf{active terms} & \textbf{sum/sum} & \textbf{n} \\
\hline
0 (all stripped) & \textbf{0.000} & 45    \\
1                & 0.170 & 315   \\
2                & 0.327 & 945   \\
3                & 0.473 & 1{,}575 \\
4                & 0.612 & 1{,}575 \\
5                & 0.745 & 945   \\
6                & 0.874 & 315   \\
7 (full)         & \textbf{1.000} & 45    \\
\hline
\end{tabular}
\caption{Sum-over-sum magnitude recovery grouped by how many process-score terms are active, on all $45$ injection sessions. Sum/sum rises steadily with the number of active terms, reaching $1.000$ under the full seven-term formula and dropping to $0.000$ when every process term is zeroed.}
\label{tab:results-ablation-monotone}
\end{table}

\paragraph{Per-term single-knob recovery.}
Table~\ref{tab:results-ablation-solo} isolates each process term by leaving that term active and zeroing the other six. On this test, \texttt{length} and \texttt{broken} carry the most magnitude, with sum/sum values of $0.328$ and $0.302$; \texttt{manualIde} follows at $0.261$. The lag term contributes less, but still has a visible effect at $0.132$, which is consistent with its role as a secondary intermediate-cleanliness penalty rather than a dominant signal. \texttt{skipTests} and \texttt{commitGap} are smaller again at $0.089$ and $0.080$. By contrast, \texttt{gain} has sum/sum $0.000$ when used alone, despite having the largest production weight. This is expected: user and alternative trajectories are validated against the same terminal AST, so their endpoint cleanliness gain is the same and cancels out in a direct comparison. The spread between \texttt{commitGap} and \texttt{length} also shows that recovery depends not only on the assigned weight, but on how much that signal actually varies across the sessions.
...
\begin{tabular}{lrr}
\hline
\textbf{term} & \textbf{mean recovery} & \textbf{sum/sum} \\
\hline
\texttt{length}    & 0.578 & \textbf{0.328} \\
\texttt{broken}    & 0.553 & 0.302          \\
\texttt{manualIde} & 0.442 & 0.261          \\
\texttt{lag}       & 0.423 & 0.132          \\
\texttt{skipTests} & 0.398 & 0.089          \\
\texttt{commitGap} & 0.368 & 0.080          \\
\texttt{gain}      & 0.311 & \textbf{0.000} \\
\hline
\end{tabular}
\caption{Per-term single-knob recovery on all $45$ injection sessions (each term active alone, the other six zeroed). The \texttt{length} and \texttt{broken} terms are the strongest single-term contributors; the lag term contributes $0.132$ on its own, fourth-largest of the seven. The \texttt{gain} term has sum/sum $0.000$ alone because user and alternative trajectories have the same terminal AST and therefore the same endpoint cleanliness gain.}
\label{tab:results-ablation-solo}
\end{table}

\paragraph{Per-term leave-one-out recovery.}
Table~\ref{tab:results-ablation-loo} shows the complementary leave-one-out view: each row removes one term from the full seven-term formula. Removing \texttt{length} causes the largest loss of magnitude, with sum/sum falling to $0.718$, so the step-savings term is the strongest contributor in this setting. Removing \texttt{manualIde}, \texttt{broken}, or \texttt{lag} also reduces recovery, but less sharply.

The unusual row is \texttt{gain}. Removing it increases sum/sum to $1.060$, meaning the absolute divergence magnitudes become larger than under the production formula. This does not mean that \texttt{gain} is harmful. Because user and alternative trajectories have the same terminal AST, the gain term is a shared endpoint component rather than a source of separation by itself. Its effect in the full formula comes through the $[0,100]$ score clamp: a shared positive or negative gain can push both trajectories towards a clamp boundary, compressing the visible difference created by the penalty terms. When \texttt{gain} is removed, that compression is reduced and the penalty differences can produce larger absolute gaps. The lag term has the opposite profile: removing it lowers sum/sum to $0.842$ while keeping the rest of the formula intact, so it contributes measurable magnitude without dominating the top of the injection-session rankings.
...
\begin{tabular}{lrr}
\hline
\textbf{removed term} & \textbf{mean recovery} & \textbf{sum/sum} \\
\hline
\texttt{length}    & 0.817 & \textbf{0.718} \\
\texttt{manualIde} & 0.911 & 0.822          \\
\texttt{broken}    & 0.966 & 0.833          \\
\texttt{lag}       & 0.880 & 0.842          \\
\texttt{skipTests} & 0.903 & 0.899          \\
\texttt{commitGap} & 0.956 & 0.943          \\
\texttt{gain}      & 0.991 & \textbf{1.060} \\
\hline
\end{tabular}
\caption{Per-term leave-one-out recovery on all $45$ injection sessions (six terms active, one removed). Removing \texttt{length} causes the largest sum/sum drop; removing \texttt{gain} inflates sum/sum above $1.000$ because the shared endpoint gain can push scores towards a clamp boundary and compress penalty-driven differences. Removing the lag term costs $0.158$ on sum/sum.}
\label{tab:results-ablation-loo}
\end{table}

\paragraph{Cross-set rerun on the user-study sessions.}
Table~\ref{tab:results-ablation-user-loo} repeats the leave-one-out test as a rank-stability comparison across the injection and user-study sessions. The $\tau$ values here are computed on the rankable subset of each set ($15$ injection sessions, $25$ user-study sessions) for the reasons stated in §\ref{sec:results-sensitivity}. On the user-study rankable subset, \texttt{length} is the clearest rank-carrier: removing it drops $\tau$ to $0.527$, the lowest value in the user-study column. \texttt{manualIde} and \texttt{broken} are the next most influential terms, while \texttt{skipTests}, \texttt{commitGap}, and \texttt{gain} sit in the middle of the range. The lag term has the smallest user-study impact ($\tau = 0.784$ on removal), suggesting that on these longer human traces it affects magnitude more than the ordering of divergence points. The injection-set $\tau$ values are lower across the board, which reflects the smaller rankable subset ($15$ sessions); the relative ordering still picks out \texttt{length} and \texttt{manualIde} as the strongest rank-carriers there too.
...
\hline
\end{tabular}
\caption{Leave-one-out ranking stability on the injection and user-study rankable subsets, sorted by user-study impact. Removing \texttt{length} causes the largest user-study drop ($\tau = 0.527$). Removing the lag term has the smallest user-study effect ($\tau = 0.784$), confirming the per-knob disruption pattern in Table~\ref{tab:results-sensitivity-knobs} that the lag term affects user-study magnitude more than ordering.}
\label{tab:results-ablation-user-loo}
\end{table}

\paragraph{Caveats.}
Several limits are worth noting. The cleanliness sub-weights are not included in this ablation because the sensitivity sweep found that they had almost no effect on top-$1$ stability (\S\ref{sec:results-sensitivity}); the sweep is therefore limited to the seven process-side weights in \S\ref{sec:methodology-score}. Score clipping appears to have only a small effect here: the mean number of ceiling-hit divergence points stays between $0.20$ and $0.29$ across variants. The magnitude metrics in Tables \ref{tab:results-ablation-monotone}, \ref{tab:results-ablation-solo}, and \ref{tab:results-ablation-loo} use all $45$ injection sessions; the rank statistic in Table~\ref{tab:results-ablation-user-loo} uses the rankable subsets ($15$ injection, $25$ user-study), as defined in §\ref{sec:results-sensitivity}.

The \texttt{gain}-alone row in Table~\ref{tab:results-ablation-solo} should also be read carefully. It sits at the all-stripped baseline (sum/sum $0.000$) because user and alternative trajectories end at the same terminal AST, so the endpoint cleanliness gain is the same and cancels out when no other process terms are active. In the leave-one-out table, \texttt{gain} matters only through its interaction with the $[0,100]$ score clamp.
...
\hline
\end{tabular}
\caption{Per-cell gain-stripped trajectory and total divergence-point count across the eight agent stacks. Cells 1--6 are with-feedback (named for the model $\times$ harness combination); cells 7 and 8 are no-feedback baselines, named with the \texttt{-baseline} suffix. The arm means aggregate across cells within the arm.}
