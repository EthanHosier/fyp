...
The direction of the participants' improvement is what the tool's feedback is designed to produce, but the design does not separate feedback exposure from codebase familiarity, and $n = 2$ participants is too small to support a causal claim. The threats-to-validity chapter discusses both of these caveats. What the table does support is a narrower claim: the production score's session-level value is dominated by the cleanliness-gain term, so the production view alone does not show what changes in the participants' process across the arc, and the gain-stripped view does.
