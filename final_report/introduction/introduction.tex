...
\begin{tikzpicture}[
  ...
  % Alternative trajectory (dashed) — diverges at step 3, then mirrors
  % the user's deltas on the diverged state (same Δ per step, parallel).
  \draw[thick, dashed] (3,50) -- (4,63) -- (5,60) -- (6,68);
  \foreach \p in {(4,63),(5,60),(6,68)}
    \draw[thick, fill=white] \p circle (0.06 and 0.6);
  \node[anchor=west] at (6.1, 68) {$s_T^{\,\mathrm{alt}}$};
  ...
  \draw[thick, dashed] (3.6, 76) -- (4.4, 76);
...
\begin{itemize}
  \item A process-quality metric for refactoring trajectories. The metric combines terms from prior work on code quality and developer process signals, and we test how much the results change under sensitivity and ablation experiments.
  ...
  \item A 45-session corpus of recorded refactoring sessions on a purpose-built Java codebase. We provide multi-label ground-truth annotations and the experimental scripts used for the sensitivity, ablation, and divergence analyses.
