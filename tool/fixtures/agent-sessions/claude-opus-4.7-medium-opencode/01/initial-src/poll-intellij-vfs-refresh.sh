...
# Heartbeat: print a summary line every N successful polls. With INTERVAL=0.3s
# and N=10 that's roughly one summary every 3 seconds — enough to confirm the
# loop is alive without flooding the terminal. VERBOSE=1 forces one line per
# poll.
SUMMARY_EVERY=10
...
while true; do
  ...
  if [[ "${status}" == "200" ]]; then
    ...
    # body looks like {"ok":true,"elapsedMs":17}
    ...
    # Warn on first failure of a streak, then every 20th, to avoid spam.
